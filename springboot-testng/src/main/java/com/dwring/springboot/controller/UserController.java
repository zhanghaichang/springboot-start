...
import com.dwring.springboot.validator.JsonSchemaValidator;
...
public class UserController {
	...
	* @param @param requesttParam
	* @param @return    设定文件 
	* @return BaseResponse<String>    返回类型 
	* @throws 
	*/
	@ApiOperation(value = "创建用户", notes = "根据User对象创建用户")
	@ApiImplicitParam(name = "user", value = "用户详细实体user", required = true, dataType = "UserInfo")
	@RequestMapping(value = "/", method = RequestMethod.POST)
	public BaseResponse<String> creatUser(@RequestBody String requesttParam) {
		// 验证JSON信息
		ObjectMapper mapper = new ObjectMapper();
		ObjectNode node;
		try {
			node = (ObjectNode) mapper.readTree(requesttParam);
			String errorMsg = jsonSchemaValidator.validatorSchema("UserInfo.json", node);
		...
		return new BaseResponse<String>(requesttParam);
	}
	...
	public BaseResponse<UserInfo> updateUser(@PathVariable Integer id, @RequestBody UserInfo user) {
