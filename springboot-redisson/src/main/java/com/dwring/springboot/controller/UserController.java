...
public class UserController {
	...
	public BaseResponse<List<UserInfo>> getUserList() {
	...
	public BaseResponse<UserInfo> creatUser(@RequestBody @Validated UserInfo user) {
		// 处理"/users/"的POST请求，用来创建User
		// 除了@ModelAttribute绑定参数之外，还可以通过@RequestParam从页面中传递参数
		userInfoService.save(user);
		return new BaseResponse<UserInfo>(user);
	}
	...
	public BaseResponse<UserInfo> loadUser(@PathVariable Integer id) {
		...
		return new BaseResponse<UserInfo>(userInfoService.getById(id));
	}
	...
	public BaseResponse<UserInfo> updateUser(@PathVariable Integer id, @RequestBody @Validated UserInfo user) {
		...
		DistributedRedisLock.acquire("user.update"+id);
		...
		DistributedRedisLock.release("user.update"+id);
